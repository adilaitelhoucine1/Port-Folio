  ...
  return (
    <div className="section-container">
      ...
      >
        ...
        >
          <div className={`inline-flex items-center space-x-2 px-6 py-3 bg-gradient-to-r ${selectedCat?.gradient} rounded-full text-white font-bold shadow-lg`}>
            {selectedCat && <selectedCat.icon className="w-5 h-5" />}
        ...
        <p className="text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
          Cutting-edge technologies for building scalable applications
        </p>
      </motion.div>
        ...
        {categories.map((cat, index) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className={`
                relative px-4 md:px-8 py-3 md:py-4 rounded-2xl font-semibold text-sm md:text-lg
                transition-all duration-300 flex items-center space-x-2 md:space-x-3 overflow-hidden
                ${isActive
                  ? `bg-gradient-to-r ${cat.gradient} text-white shadow-2xl shadow-${cat.gradient}/50`
            ...
            return (
              ...
              >
                ...
                <div className="relative h-full bg-white dark:bg-gray-800 rounded-xl md:rounded-2xl p-4 md:p-6 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 border-gray-100 dark:border-gray-700 hover:border-transparent overflow-hidden">
                  ...
                  {isHovered && (
                    <motion.div
                      className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${tech.color} opacity-20 blur-xl`}
                      animate={{
                        scale: [1, 1.1, 1],
                        rotate: [0, 5, 0],
                      }}
                  ...
                  <div className="relative z-10">
                    ...
                    <motion.div
                      ...
                      className="mt-3 md:mt-4 hidden md:flex items-center justify-center space-x-2 text-xs font-semibold"
                    >
        ...
        <div className="inline-flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-8 px-6 md:px-8 py-4 md:py-6 bg-gradient-to-r from-primary/10 via-secondary/10 to-accent/10 rounded-xl md:rounded-2xl border-2 border-white/20 dark:border-white/10">
          <div className="text-center">
            ...
            <div className="text-xs md:text-sm text-gray-600 dark:text-gray-400 font-medium">
              {selectedCategory === 'all' ? 'Total Technologies' : `${categories.find(c => c.id === selectedCategory)?.name}`}
            </div>
          </div>
